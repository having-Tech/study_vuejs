...
<div class="container">
  ...
  <ul class="list-group">
    <li class="list-group-item" v-bind:key="todo" v-for="todo in todos">
      {{todo.name}}
    ...
    methods:{
      deleteTodo(todo){
        ...
        this.todos.forEach(function(_todo,i, obj){
          if(_todo.id === todo.id){
            vm.$http.delete('http://todos.garam.xyz/api/todos/'+todo.id).then((result)=> { obj.splice(i,1)})
          }
        })
      },
      createTodo(name){
      ...
      getTods(){
        var vm = this;
        this.$http.get('http://todos.garam.xyz/api/todos').the((result) => {console.log(result)})
      }
    },
    mounted() {
      this.getTodos();
